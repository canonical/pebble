...
func (cs *clientSuite) TestClientIntegrationHTTPS(c *C) {
	...
	c.Assert(err, IsNil)
	_, err = cli.SysInfo()
	c.Assert(err, ErrorMatches, ".*cannot verify server: missing client TLS config.*")

	// 2. Let's simulate a pairing attempt by supplying the server
	...
	// 3. Let's simulate a normal TLS request by supplying the server
