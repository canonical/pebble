...
Pebble supports FIPS 140-compliant builds using the `fips` build tag. When built with `-tags=fips`, Pebble will not include the third-party [github.com/GehirnInc/crypt](https://github.com/Gehirninc/crypt) library, and basic authentication using password hashes will be disabled. Only "local" (UID-based) and "cert" (certificate-based) identity types will be available in FIPS mode.
...
When running in FIPS mode, attempting to add or use basic authentication identities will return an error indicating that basic authentication is not supported in FIPS mode.
