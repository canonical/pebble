...
func (s *PebbleSuite) TestEnterSubCommandWaits(c *C) {
	// The service writes a sentinel value to a file on startup. The
	// subcommand cats that file: if enter correctly waited for the
	// service to start before running the subcommand, cat succeeds
	// and outputs the sentinel; otherwise the file does not yet exist
	// and cat fails.
	layerTemplate := dumbDedent(`
		services:
		  stat:
		    override: replace
		    command: /bin/sh -c 'echo started > $PEBBLE/enter-wait; sleep 5'
		    startup: enabled
	`)
	layerPath := filepath.Join(s.pebbleDir, "layers", "001-stat.yaml")
	writeTemplate(layerPath, layerTemplate, nil)

	cmd := []string{"pebble", "enter", "--run", "exec", "cat",
		filepath.Join(s.pebbleDir, "enter-wait")}
	restore := fakeArgs(cmd...)
	defer restore()
	...
	// see cmd/pebble/cmd_exec.go:163
	c.Check(s.Stderr(), Equals, "")
	c.Check(s.Stdout(), Equals, "started\n")
}
