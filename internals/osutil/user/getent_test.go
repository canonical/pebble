...
func (s *getentSuite) SetUpTest(c *C) {
	...
	s.mockGetent = testutil.FakeCommand(c, "getent", fmt.Sprintf(`
set -eu
base='%s'/"${1}${2:+/}${2-}"
cat "${base}"
if [ -f "${base}.exit" ]; then
...
`, s.getentDir))
	s.AddCleanup(user.FakeGetentSearchPath(s.mockGetent.BinDir() + ":" + user.DefaultGetentSearchPath))
...
func (s *getentSuite) TestNoGetentBinary(c *C) {
	defer user.FakeGetentSearchPath("/foo:/bar")()
