...
import (
	...
	"github.com/canonical/pebble/internals/osutil/strace"
...
type straceSuite struct {
	rootdir    string
	mockSudo   *testutil.FakeCmd
	mockStrace *testutil.FakeCmd
}
...
func (s *straceSuite) SetUpTest(c *C) {
	...
	s.mockSudo = testutil.FakeCommand(c, "sudo", "")
	s.mockStrace = testutil.FakeCommand(c, "strace", "")
}
...
func (s *straceSuite) TestStraceCommandHappyFromSnap(c *C) {
	...
	c.Assert(err, IsNil)
	mockStraceStatic := testutil.FakeCommand(c, straceStaticPath, "")
