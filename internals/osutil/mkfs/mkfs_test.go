...
import (
	...
	"github.com/canonical/pebble/internals/osutil/mkfs"
...
func (m *mkfsSuite) SetUpTest(c *C) {
	...
	// host tools
	cmdFakeroot := testutil.FakeCommand(c, "fakeroot", "echo 'override in test' ; exit 1")
	m.AddCleanup(cmdFakeroot.Restore)

	cmdMkfsExt4 := testutil.FakeCommand(c, "mkfs.ext4", "echo 'override in test' ; exit 1")
	m.AddCleanup(cmdMkfsExt4.Restore)

	cmdMkfsVfat := testutil.FakeCommand(c, "mkfs.vfat", "echo 'override in test'; exit 1")
	m.AddCleanup(cmdMkfsVfat.Restore)

	cmdMcopy := testutil.FakeCommand(c, "mcopy", "echo 'override in test'; exit 1")
...
func (m *mkfsSuite) TestMkfsExt4Happy(c *C) {
	...
	if useFakeroot {
		cmd = testutil.FakeCommand(c, "fakeroot", "")
	} else {
		cmd = testutil.FakeCommand(c, "mkfs.ext4", "")
	}
	defer cmd.Restore()
...
func (m *mkfsSuite) TestMkfsExt4Error(c *C) {
	...
	var cmd *testutil.FakeCmd
	if useFakeroot {
		cmd = testutil.FakeCommand(c, "fakeroot", "echo 'command failed'; exit 1")
	} else {
		cmd = testutil.FakeCommand(c, "mkfs.ext4", "echo 'command failed'; exit 1")
	}
	defer cmd.Restore()
...
func (m *mkfsSuite) testMkfsVfatHappySimple(c *C, fatType, fatBits string) {
	...
	cmd := testutil.FakeCommand(c, "mkfs.vfat", "")
	defer cmd.Restore()
...
func (m *mkfsSuite) TestMkfsVfatHappyContents(c *C) {
	...
	cmdMkfs := testutil.FakeCommand(c, "mkfs.vfat", "")
	defer cmdMkfs.Restore()

	cmdMcopy := testutil.FakeCommand(c, "mcopy", "")
...
func (m *mkfsSuite) TestMkfsVfatErrorSimpleFail(c *C) {
	...
	cmd := testutil.FakeCommand(c, "mkfs.vfat", "echo 'failed'; false")
	defer cmd.Restore()
...
func (m *mkfsSuite) TestMkfsVfatErrorInMcopy(c *C) {
	...
	cmdMcopy := testutil.FakeCommand(c, "mcopy", "echo 'hard fail'; exit 1")
	defer cmdMcopy.Restore()
