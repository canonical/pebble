...
func TestMountinfoSuite(t *testing.T) {
	tc.Run(t, &mountinfoSuite{})
}

// Check that parsing the example from kernel documentation works correctly.
func (s *mountinfoSuite) TestParseMountInfoEntry1(c *tc.C) {
	...
	c.Assert(entry.ParentID, tc.Equals, 35)
	c.Assert(entry.DevMajor, tc.Equals, 98)
	c.Assert(entry.DevMinor, tc.Equals, 0)
	c.Assert(entry.Root, tc.Equals, "/mnt1")
	c.Assert(entry.MountDir, tc.Equals, "/mnt2")
	...
	c.Assert(entry.MountSource, tc.Equals, "/dev/root")
	c.Assert(entry.SuperOptions, tc.DeepEquals, map[string]string{"rw": "", "errors": "continue"})
}

// Check that various combinations of optional fields are parsed correctly.
func (s *mountinfoSuite) TestParseMountInfoEntry2(c *tc.C) {
	// No optional fields.
	...
	c.Assert(entry.MountOptions, tc.DeepEquals, map[string]string{"rw": "", "noatime": ""})
	c.Assert(entry.OptionalFields, tc.HasLen, 0)
	c.Assert(entry.FsType, tc.Equals, "ext3")
	// One optional field.
...
func (s *mountinfoSuite) TestParseMountInfoEntry3(c *tc.C) {
...
func (s *mountinfoSuite) TestLoadMountInfo2(c *tc.C) {
	fname := filepath.Join(c.MkDir(), "mountinfo")
