...
import (
	...
	"github.com/canonical/pebble/internals/osutil/kmod"
...
func (s *kmodSuite) TestModprobeCommandFailure(c *C) {
	cmd := testutil.FakeCommand(c, "modprobe", "exit 1")
	defer cmd.Restore()
...
func (s *kmodSuite) TestModprobeCommandHappy(c *C) {
	cmd := testutil.FakeCommand(c, "modprobe", "")
	defer cmd.Restore()
...
func (s *kmodSuite) TestLoadModule(c *C) {
	...
	restore := kmod.FakeModprobeCommand(func(args ...string) error {
