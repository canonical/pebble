...
func (s *nfsSuite) TestIsHomeUsingRemoteFS(c *C) {
	...
	for _, tc := range cases {
		restore := osutil.FakeMountInfo(tc.mountinfo)
		defer restore()
		restore = osutil.FakeEtcFstab(tc.fstab)
		defer restore()
...
func (s *nfsSuite) TestSnapDirsUnderNFSMounts(c *C) {
	restore := osutil.FakeSyscallStatfs(func(path string, statfs *syscall.Statfs_t) error {
	...
	dirPath := c.MkDir()
	restore = osutil.FakeAllDataHomeGlobs(func() []string {
