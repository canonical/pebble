...
func (s *mockDiskSuite) TestMockDeviceNameToDiskMapping(c *C) {
	...
	m := map[string]*disks.FakeDiskMapping{
		"devName1":   d1,
		"devName2":   d1,
		"other-disk": d2,
	}

	r := disks.FakeDeviceNameToDiskMapping(m)
	defer r()
...
func (s *mockDiskSuite) TestMockDevicePathToDiskMapping(c *C) {
	...
	r := disks.FakeDevicePathToDiskMapping(m)
	defer r()
...
func (s *mockDiskSuite) TestMockPartitionDeviceNodeToDiskMapping(c *C) {
	...
	r := disks.FakePartitionDeviceNodeToDiskMapping(m)
	defer r()
...
func (s *mockDiskSuite) TestMockMountPointDisksToPartitionMappingVerifiesUniqueness(c *C) {
	...
	m := map[disks.Mountpoint]*disks.FakeDiskMapping{
	...
	// mocking works
	r := disks.FakeMountPointDisksToPartitionMapping(m)
	defer r()

	// changing so they have the same DevNum doesn't work though
	d2.DevNum = "d1"
	c.Assert(
		func() { disks.FakeMountPointDisksToPartitionMapping(m) },
		PanicMatches,
	...
	m2 := map[disks.Mountpoint]*disks.FakeDiskMapping{
		{Mountpoint: "mount1"}: d1,
		{Mountpoint: "mount2"}: d1,
	}
	r = disks.FakeMountPointDisksToPartitionMapping(m2)
	defer r()
}
...
func (s *mockDiskSuite) TestMockMountPointDisksToPartitionMapping(c *C) {
	...
	r := disks.FakeMountPointDisksToPartitionMapping(
		map[disks.Mountpoint]*disks.FakeDiskMapping{
			{Mountpoint: "mount1"}: d1,
			{Mountpoint: "mount2"}: d1,
			{Mountpoint: "mount3"}: d2,
		},
