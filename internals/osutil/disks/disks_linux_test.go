...
import (
	...
	"github.com/canonical/pebble/internals/osutil/internal/gadgettest"
...
func (s *diskSuite) TestDiskFromDeviceNameHappy(c *C) {
	...
	restore := disks.FakeUdevPropertiesForDevice(func(typeOpt, dev string) (map[string]string, error) {
...
func (s *diskSuite) TestDiskFromDevicePathHappy(c *C) {
	...
	mockUdevadm := testutil.FakeCommand(c, "udevadm", ``)
...
func (s *diskSuite) TestDiskFromMountPointUnhappyMissingUdevProps(c *C) {
	restore := osutil.FakeMountInfo(`130 30 42:1 / /run/mnt/point rw,relatime shared:54 - ext4 /dev/vda4 rw
`)
	defer restore()
...
func (s *diskSuite) TestDiskFromMountPointUnhappyIsDecryptedDeviceNoSysfs(c *C) {
	restore := osutil.FakeMountInfo(`130 30 252:0 / /run/mnt/point rw,relatime shared:54 - ext4 /dev/mapper/something rw
`)
	defer restore()
...
func (s *diskSuite) TestDiskFromMountPointHappySinglePartitionIgnoresNonPartitionsInSysfs(c *C) {
	...
	restore := osutil.FakeMountInfo(`130 30 47:1 / /run/mnt/point rw,relatime shared:54 - ext4 /dev/vda4 rw
`)
	defer restore()
...
func (s *diskSuite) TestDiskFromMountPointHappyRealUdevadm(c *C) {
	restore := osutil.FakeMountInfo(`130 30 42:1 / /run/mnt/point rw,relatime shared:54 - ext4 /dev/vda1 rw
`)
	defer restore()

	udevadmCmd := testutil.FakeCommand(c, "udevadm", fmt.Sprintf(`
...
func (s *diskSuite) TestDiskFromMountPointVolumeUnhappyWithoutPartEntryDisk(c *C) {
	restore := osutil.FakeMountInfo(`130 30 42:1 / /run/mnt/point rw,relatime shared:54 - ext4 /dev/mapper/something rw
`)
	defer restore()

	udevadmCmd := testutil.FakeCommand(c, "udevadm", `
...
func (s *diskSuite) TestDiskFromMountPointIsDecryptedLUKSDeviceVolumeHappy(c *C) {
	restore := osutil.FakeMountInfo(`130 30 242:1 / /run/mnt/point rw,relatime shared:54 - ext4 /dev/mapper/something rw
`)
	defer restore()
...
func (s *diskSuite) TestDiskFromMountPointNotDiskUnsupported(c *C) {
	restore := osutil.FakeMountInfo(`130 30 42:1 / /run/mnt/point rw,relatime shared:54 - ext4 /dev/not-a-disk rw
`)
	defer restore()
...
func (s *diskSuite) TestDiskFromMountPointUnsupportedSchema(c *C) {
	restore := osutil.FakeMountInfo(`130 30 42:1 / /run/mnt/point rw,relatime shared:54 - ext4 /dev/not-a-supported-schema-disk rw
`)
	defer restore()
...
func (s *diskSuite) TestDiskFromMountPointPartitionsHappy(c *C) {
	...
	restore := osutil.FakeMountInfo(`130 30 42:4 / /run/mnt/data rw,relatime shared:54 - ext4 /dev/vda4 rw
...
func (s *diskSuite) TestDiskFromMountPointDecryptedDevicePartitionsHappy(c *C) {
	...
	restore := osutil.FakeMountInfo(`130 30 252:0 / /run/mnt/data rw,relatime shared:54 - ext4 /dev/mapper/ubuntu-data-3776bab4-8bcc-46b7-9da2-6a84ce7f93b4 rw
 130 30 42:4 / /run/mnt/ubuntu-boot rw,relatime shared:54 - ext4 /dev/vda3 rw
`)
	defer restore()
...
func (s *diskSuite) TestMountPointsForPartitionRoot(c *C) {
	...
	for _, t := range tt {
		...
		restore := osutil.FakeMountInfo(t.mountinfo)
...
func (s *diskSuite) TestDiskSizeRelatedMethodsGPT(c *C) {
	...
	restoreCalculateLBA := disks.FakeCalculateLastUsableLBA(42, nil)
	defer restoreCalculateLBA()

	sfdiskCmd := testutil.FakeCommand(c, "sfdisk", `
if [ "$1" = --version ]; then
...
`)
	...
	blockDevCmd := testutil.FakeCommand(c, "blockdev", `
if [ "$1" = "--getsize64" ]; then
	echo 5120000
elif [ "$1" = "--getss" ]; then
	echo 512
...
func (s *diskSuite) TestDiskSizeRelatedMethodsGPTFallback(c *C) {
	...
	restoreCalculateLBA := disks.FakeCalculateLastUsableLBA(0, errors.New("Some error"))
	defer restoreCalculateLBA()
...
func (s *diskSuite) TestDiskUsableSectorsEndGPTUnexpectedSfdiskUnit(c *C) {
	...
	cmd := testutil.FakeCommand(c, "sfdisk", `
if [ "$1" = --version ]; then
	echo 'sfdisk from util-linux 2.34.1'
	exit 0
fi
...
func (s *diskSuite) TestDiskSizeRelatedMethodsDOS(c *C) {
	...
	sfdiskCmd := testutil.FakeCommand(c, "sfdisk", "echo broken test; exit 1")
	defer sfdiskCmd.Restore()
...
func (s *diskSuite) TestAllPhysicalDisks(c *C) {
	...
	restore := disks.FakeUdevPropertiesForDevice(func(typ, dev string) (map[string]string, error) {
...
func (s *diskSuite) TestFindMatchingPartitionWithFsLabel(c *C) {
	// mock disk
	restore := disks.FakeDeviceNameToDiskMapping(map[string]*disks.FakeDiskMapping{
...
func (s *diskSuite) TestMockDisksChecking(c *C) {
	f := func() {
		disks.FakeDeviceNameToDiskMapping(map[string]*disks.FakeDiskMapping{
			"/dev/vda": {
