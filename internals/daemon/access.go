...
// end-point. This access checker allows a new mTLS client identity to be
...
// pairing window enabled, which typically involes a proof of server ownership
...
func (ac PairingAccess) CheckAccess(d *Daemon, r *http.Request, user *UserState) Response {
	...
	if pairingWindowEnabled(d) {
		...
		// manager uncessarily. The final check is made inside the
