...
func v1GetNotices(c *Command, r *http.Request, _ *UserState) Response {
	...
	if len(query["users"]) > 0 {
		if !isAdmin(requestUID, daemonUID) {
			return statusForbidden(`only admins may use the "users" filter`)
		}
		if len(query["user-id"]) > 0 {
			return statusBadRequest(`cannot use both "users" and "user-id" parameters`)
		}
		if query.Get("users") != "all" {
			return statusBadRequest(`invalid "users" filter: must be "all"`)
		}
