...
// Package opentelemetry implements a metricsstate client that forwards enriched
// OTLP metrics to a remote OpenTelemetry/HTTP collector.
...
// Client sends enriched OTLP metrics to a remote OpenTelemetry/HTTP collector,
...
// NewClient creates a new Client for the metricstarget described by options.
...
// Add adds the given batch of resource metrics to the client's buffer, to be
// enriched and sent on the next Flush.
func (c *Client) Add(service, instanceID string, labels map[string]string, resourceMetrics []*metricspb.ResourceMetrics) error {
	if len(c.batches) >= c.options.MaxBufferedBatches {
		// Buffer is full - drop the oldest batch to make room.
		logger.Noticef("Target %q: metrics buffer full, dropping oldest batch", c.options.TargetName)
		c.batches[0] = batch{}
...
// Flush sends the buffered metrics to the OpenTelemetry collector.
func (c *Client) Flush(ctx context.Context) error {
	if len(c.batches) == 0 {
		return nil
	}

	resourceMetrics := make([]*metricspb.ResourceMetrics, 0, len(c.batches))
	for _, b := range c.batches {
		for _, rm := range b.resourceMetrics {
			resourceMetrics = append(resourceMetrics, enrich(rm, b.service, b.instanceID, b.labels))
		}
	}
...
// enrich returns a copy of rm with its resource attributes merged with the
// standard Pebble enrichment attributes and the target's custom labels. The
// original ResourceMetrics (and its nested ScopeMetrics) are not mutated, since
// they may be shared with other targets' clients.
func enrich(rm *metricspb.ResourceMetrics, service, instanceID string, labels map[string]string) *metricspb.ResourceMetrics {
...
func (c *Client) sendBatch(ctx context.Context, data *metricspb.MetricsData) error {
	protoData, err := proto.Marshal(data)
	if err != nil {
		return fmt.Errorf("cannot marshal metrics batch: %v", err)
	}

	req, err := http.NewRequestWithContext(ctx, "POST", c.options.Location+"/v1/metrics", bytes.NewReader(protoData))
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("cannot send metrics: %v", err)
	}
