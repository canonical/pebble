...
func (ts *tlsSuite) TestTLSServerClient(c *C) {
	...
	for _, session := range httpsSessions {
		restore, clock := mgr.FakeSystemTime(session, 0)

		// Test an untrusted client connection.
		certs, err := ts.testTLSInsecureClient(c, clock)
		c.Assert(err, IsNil)

		ca := certs[1] // First one is the TLS cert.

		// Test a trusted client connection (we use the self-signed identity CA cert).
