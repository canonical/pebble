...
func (*suite) TestFlushPropagatesTrace(c *C) {
	...
	client := opentelemetry.NewClient(&opentelemetry.ClientOptions{Location: location})
	...
	c.Check(attrs["url.full"], Equals, strings.Replace(server.URL, "http://", "http://user:xxxxx@", 1)+"/v1/logs")
	c.Check(attrs["http.response.status_code"], Equals, int64(200))
