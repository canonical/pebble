...
// testClient is a fake tracesClient implementation used for testing the
// gatherer's buffering and flushing behaviour.
type testClient struct {
	mu      sync.Mutex
	added   []tracesBatch
	sendCh  chan []tracesBatch
	flushed int
}

func (t *testClient) Add(service, instanceID string, labels map[string]string, resourceSpans []*tracepb.ResourceSpans) error {
	t.mu.Lock()
	defer t.mu.Unlock()
	t.added = append(t.added, tracesBatch{
		service:       service,
...
func someResourceSpans() []*tracepb.ResourceSpans {
	return []*tracepb.ResourceSpans{{}}
}

func (s *gathererSuite) TestGatherer(c *C) {
	received := make(chan []tracesBatch, 1)
	options := traceGathererOptions{
		maxBufferedBatches: 2,
		newClient: func(target *plan.TraceTarget) (tracesClient, error) {
			return &testClient{sendCh: received}, nil
		},
	}

	g, err := newtraceGathererInternal(&plan.TraceTarget{Name: "tgt1"}, &options)
	c.Assert(err, IsNil)
	defer g.Stop()

	g.Add("svc1", "instance-1", nil, someResourceSpans())
	g.Add("svc1", "instance-1", nil, someResourceSpans())
...
func (s *gathererSuite) TestGathererDropsWhenBufferFull(c *C) {
	...
	g.Add("svc1", "", nil, someResourceSpans())
	time.Sleep(10 * time.Millisecond)
	g.Add("svc1", "", nil, someResourceSpans())
	g.Add("svc1", "", nil, someResourceSpans())
...
func (b *blockingTestClient) Add(service, instanceID string, labels map[string]string, resourceSpans []*tracepb.ResourceSpans) error {
	b.once.Do(func() {
		<-b.unblock
	})
	b.mu.Lock()
	defer b.mu.Unlock()
	b.added = append(b.added, tracesBatch{service: service})
	return nil
}
